...
// Resolved "Unexpected any" by defining strict interfaces
interface Stitcher {
  id: number;
  name: string;
  phone: string;
...
interface OrderItem {
  ...
  status: string;
  order_id: number;
  stitcher_id?: number; // Added to handle filtering safely
}
...
export default function StitcherProfile() {
  ...
  // Wrapped in useCallback to fix "missing dependencies" warning
  const fetchStitcherData = useCallback(async () => {
    if (!id) return;
    
    try {
      setIsLoading(true);
      // Fetch stitcher details with explicit typing
      const res = await apiClient.get<Stitcher>(`/stitchers/${id}`);
      setStitcher(res.data);
      
      // Fetch items with explicit typing
      const itemsRes = await apiClient.get<OrderItem[]>('/orders/items/');
      
      // Removed 'any' from the filter logic
      const assignedItems = itemsRes.data.filter(
        (item: OrderItem) => item.stitcher_id === parseInt(id || '0')
      );
      setItems(assignedItems);
    } catch (error) {
      toast({
  ...
  }, [id, navigate, toast]); // Added required dependencies
