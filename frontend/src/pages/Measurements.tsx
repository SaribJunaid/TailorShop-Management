...
// --- Interfaces to fix "Unexpected any" ---
interface MeasurementsData {
  ...
  pant_length?: string;
  coat_length?: string;
  [key: string]: string | number | undefined; // Allow string indexing for the loop
}
...
export default function Measurements() {
  ...
  const [formData, setFormData] = useState<Partial<MeasurementsData>>({});
  const [isSaving, setIsSaving] = useState(false);

  // --- Wrapped in useCallback to fix missing dependency in useEffect ---
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const custRes = await apiClient.get<Omit<Customer, 'measurements'>[]>('/customers/');
      const customersData = custRes.data;

      const customersWithM = await Promise.all(
        customersData.map(async (c) => {
          const mRes = await apiClient.get<MeasurementsData[]>(`/measurements/customer/${c.id}`);
          return { ...c, measurements: mRes.data[0] || null };
        })
      );
  ...
  }, [toast]); // toast is a stable dependency from the hook
  ...
  const measurementFields: { key: keyof MeasurementsData; label: string }[] = [
    { key: 'neck', label: 'Neck' },
    { key: 'chest', label: 'Chest' },
    { key: 'waist', label: 'Waist' },
    { key: 'shoulder', label: 'Shoulder' },
