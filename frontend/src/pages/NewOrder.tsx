...
export default function NewOrder() {
  ...
  // --- REQUIREMENT: FETCH EXISTING MEASUREMENTS ---
  useEffect(() => {
    const fetchExistingMeasurements = async () => {
      if (!selectedCustomer) return;
      try {
        // Replace with your actual endpoint to get customer measurements
        const res = await apiClient.get(`/measurements/customer/${selectedCustomer.id}`);
        if (res.data) {
          // Flatten measurement data into the key-value state
          setMeasurements(res.data.data_values || {}); 
          toast({ description: "Loaded existing measurements." });
        }
      } catch (err) {
        console.log("No existing measurements found for this customer.");
        setMeasurements({}); // Clear for new customer
      }
    };
    fetchExistingMeasurements();
  }, [selectedCustomer]);

  // --- UPDATED PDF GENERATION (Quantity instead of Status) ---
  const generatePDF = (order: any, customerName: string, selectedGarments: GarmentSelection[]) => {
    const doc = new jsPDF();
    const date = new Date().toLocaleDateString();
    ...
    autoTable(doc, {
      startY: 60,
      head: [['Garment Type', 'Quantity', 'Price (Per Unit)']],
      body: selectedGarments.map((g: GarmentSelection) => {
          const unitPrice = parseFloat(orderDetails.totalAmount) / selectedGarments.reduce((s, x) => s + x.quantity, 0);
          return [
            g.type.toUpperCase(),
            g.quantity.toString(),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Step 1: Save Measurements first if they were updated/new
      const measRes = await apiClient.post('/measurements/', {
          customer_id: selectedCustomer.id,
          garment_type: selectedGarments[0].type, // Usually based on first garment
      ...
      const payload = {
        ...
        items: selectedGarments.flatMap(g => 
          Array(g.quantity).fill(null).map(() => ({
            garment_type: g.type,
            price: parseFloat(orderDetails.totalAmount) / totalItems,
            measurement_id: measRes.data.id, // Link to the newly saved/updated measurements
            notes: orderDetails.fabricNotes,
            status: "pending"
          }))
        )
