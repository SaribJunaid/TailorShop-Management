...
// --- Interface to fix "Unexpected any" ---
...
export default function Customers() {
  ...
  const [newCust, setNewCust] = useState<CustomerPayload>({ name: '', phone: '', email: '' });
  ...
  // Use useMemo for filtering to improve performance and remove a secondary useEffect
  ...
  const handleDeleteCustomer = async (id: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm("Are you sure you want to delete this customer? This will also remove all their records.")) return;
    ...
    <Layout title="Customers">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCustomers.map((customer) => (
          <div
            key={customer.id}
            className="group bg-card rounded-2xl shadow-sm p-6 hover:shadow-md cursor-pointer border border-border hover:border-primary/20 transition-all relative"
            onClick={() => navigate(`/customers/${customer.id}`)}
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-4">
